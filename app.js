...
let carrito = [gpuLowEnd, gpuMidEnd, gpuHighEnd, cpuLowEndIntel, cpuMidEndIntel, cpuHighEndIntel, cpuLowEndAMD, cpuMidEndAMD, cpuHighEndAMD, ram8gb, ram16gb, ram32gb, ssdLowEnd, ssdMidEnd, ssdHighEnd, hddLowEnd, hddMidEnd, hddHighEnd, motherboardAMD, motherboardIntel, fuenteLowEnd, fuenteMidEnd, fuenteHighEnd, gabinete];
let carritoParaDom = [];
let parrafoProductos = document.createElement("p");
let parrafoCarrito = document.createElement("p");

// parrafoProductos.innerHTML = carrito.map(function (productos) { return (productos.nombre + " " + productos.precio + " " + productos.stock) }).join("<br>");
// document.body.append(parrafoProductos); En esta trate de hacerlo todo de una con un .map pero no me funciono

// carrito.forEach(function (productos) {
//     parrafoProductos.innerHTML = productos.nombre + " " + productos.precio + " " + productos.stock;
//     document.body.appendChild(parrafoProductos); en esta trate de hacerlo con un .forEach pero tampoco me funciono, me muestra un error de appendChild.
// });
// LAS DOS FUNCIONES DE ARRIBA SON LAS QUE TENGO DUDA, NO SE COMO HACER PARA QUE ME MUESTRE LOS PRODUCTOS EN EL HTML

for (const producto of carrito) {
    let li = document.createElement("li");
    li.innerHTML = producto.nombre + " " + producto.precio + " " + producto.stock;
    parrafoProductos.appendChild(parrafoProductos);
}
